  ...
  triggers = {
    command = format(local.template_command,
      join(" \\ \n  ", compact([
        ...
        try(length(each.value["note"]["text"]), 0) > 0 || length(var.note_suffix) > 0 ? format("--note Text=\"%s%s\",UpdatedBy=%s",
          replace(each.value["note"]["text"], "\"", "'"),
          var.note_suffix,
          try(each.value["note"]["updated_by"], var.default_note_updated_by)
        ) : "",
